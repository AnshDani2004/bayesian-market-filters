...
| **Exploratory StatArb (BTC/ETH)** | -2.63 | -0.02% | N/A | -0.01% |
...
| **Exploratory StatArb (BTC/ETH)**** | -5.02 | -0.34% | N/A | -0.34% |
...
*\*\*Diagnostic Note on StatArb Performance: The BTC/ETH pairs trading strategy yields negative Sharpe ratios despite the Kalman Filter utilizing a dynamic, recursive Robbins-Monro estimation for its process and observation noise. While Augmented Dickey-Fuller tests confirm the spread is genuinely cointegrated, the trading logic is overpowered by fee drag. The spread amplitude is incredibly tight, meaning that even when the Z-score triggers the Dynamic Kelly Sizing, the gross edge generated by the reversion is systematically consumed by the 1.5 bps maker fee required to cross the spreads on both legs. This exploratory extension successfully demonstrates the mathematical framework of Pairs Trading, but highlights that high-frequency StatArb strictly requires fee-tier advantages or high-volatility spread regimes to survive friction.*
