...
class StochasticVolParticleFilter:
    ...
    def __init__(self, 
                 ...
                 R: float = 1.0,      # Observation noise variance
                 random_seed: int = 42):
        self.N = n_particles
        self.dt = dt
        self.Q_h = Q_h
    ...
    def update(self, y: float):
        ...
        """
        # Calculate likelihood of observation y given each particle's mu
        # y ~ N(mu, R)
        likelihood = norm.pdf(y, loc=self.mu, scale=np.sqrt(self.R))
