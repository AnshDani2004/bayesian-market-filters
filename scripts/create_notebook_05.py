...
md_intro = """# Tier 1 Capstone: Multi-Asset Statistical Arbitrage (Pairs Trading)

In this notebook, we elevate the architecture to an institutional-grade **Multi-Asset Statistical Arbitrage** strategy. 
Instead of betting on the directional movement of a single asset, we bet on the mean-reverting relationship (Cointegration) between two highly correlated assets: **BTC** and **ETH**.

1. **Cointegration Kalman Filter**: We track the dynamic hedge ratio ($\beta$) and intercept ($\alpha$) between the log-prices of BTC and ETH.
2. **Spread Z-Score**: We calculate the deviation of the true price from the Kalman fair value, normalized by the Particle Filter's stochastic volatility.
3. **Dynamic Kelly Sizing**: Instead of a binary 0 or 1 allocation, we dynamically scale capital exposure proportionally to the statistical edge (Z-Score) and inversely proportional to volatility.
"""
...
code_fetch = """# 1. Fetch Data for Multiple Assets
...
print(f"Aligned {len(df)} candles. Date range: {df['timestamp'].min()} to {df['timestamp'].max()}")
"""

code_run_filters = """# 2. Cointegration Filtering & Dynamic Kelly Sizing
print("Running Pairs Kalman Filter and Particle Filter...")

kf = PairsKalmanFilter(delta=1e-4, R=1e-3)
pf = StochasticVolParticleFilter(n_particles=1000, initial_price=prices_btc[0], Q_h=0.0001)
...
for t in range(len(df)):
    ...
    # 1. Update Particle Filter to get volatility of the dependent asset (BTC)
    pf.step(y_price)
    volatility = np.sqrt(pf.get_variance())
    
    # 2. Update Pairs Kalman Filter
    ...
    # Normalize by stochastic volatility
...
code_backtest = """# 3. Backtest StatArb Strategy
...
# Fee Drag: 0% maker fee applied to both legs of the pair whenever position size changes
...
with open('/Users/ansh/Proj_Res_3/notebook_05_multi_asset_statarb.ipynb', 'w') as f:
    nbf.write(nb, f)
